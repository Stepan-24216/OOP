...
javac -d build/classes src/main/java/com/example/heapsort/*.java
...
# Компилируем тесты
javac -cp "build/classes:lib/junit-platform-console-standalone-1.9.2.jar" -d build/test src/test/java/com/example/heapsort/*.java
...
java -jar lib/junit-platform-console-standalone-1.9.2.jar \
    ...
    --select-class com.example.heapsort.HeapSortTest \
    --details summary
