...
public class HtmlReportGenerator {
    ...
    public String generate(List<StudentReport> reports) {
        ...
        for (StudentReport report : reports) {
            reportsByNick.put(report.getStudent().githubNick(), report);
        }

        for (Group group : config.getGroups().values()) {
            sb.append("<section class='group'>\n");
            sb.append("<h2 class='group-title'>Group: ").append(escape(group.name()))
                .append("</h2>\n");

            for (Student student : group.students()) {
                StudentReport report = reportsByNick.remove(student.githubNick());
                if (report != null) {
    ...
    private String renderStudentSection(StudentReport report) {
        ...
        sb.append("<h2>").append(escape(student.fullName()))
            .append(" (<a href='").append(escape(student.repoUrl())).append("'>")
            .append(escape(student.githubNick())).append("</a>)</h2>\n");
        sb.append("<table>\n");
        sb.append("<thead><tr>")
        ...
        sb.append("<p><strong>Total score: ").append(report.getTotalScore()).append("</strong>")
            .append(" — Grade: <strong>").append(scoreCalculator.toGrade(report.getTotalScore()))
            .append("</strong></p>\n");
        sb.append("</section>\n");
        return sb.toString();
    }
    ...
    private String renderCheckPointSummary(List<StudentReport> reports) {
        ...
        for (CheckPoint cp : config.getCheckPoints()) {
            sb.append("<th>").append(escape(cp.name())).append("<br/><small>")
                .append(cp.date()).append("</small></th>");
        }
        sb.append("<th>Final Grade</th></tr></thead>\n<tbody>\n");

        for (StudentReport report : reports) {
            sb.append("<tr><td>").append(escape(report.getStudent().fullName())).append("</td>");
            for (CheckPoint cp : config.getCheckPoints()) {
            ...
            sb.append("<td>").append(scoreCalculator.toGrade(report.getTotalScore()))
                .append("</td>");
            sb.append("</tr>\n");
        }
    ...
    private int scoreAtCheckPoint(StudentReport report, CheckPoint cp) {
        return report.getTaskResults().stream()
            .filter(r -> {
                ...
                return deadline == null || !deadline.isAfter(cp.date());
            })
